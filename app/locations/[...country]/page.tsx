...
export default function CountryPage({ params }: { params: { country: string } }) {
  ...
  const [locations, setLocations] = useState([
    ...
    { name: "Minneapolis-Saint Paul International Airport (MSP)", type: "airport" },
    { name: "Las Vegas McCarran International Airport (LAS)", type: "airport" },
    { name: "Detroit Metropolitan Airport (DTW)", type: "airport" },
    { name: "Atlanta Hartsfield-Jackson International Airport (ATL)", type: "airport" },
    { name: "Charlotte Douglas International Airport (CLT)", type: "airport" },
    { name: "Honolulu Daniel K. Inouye International Airport (HNL)", type: "airport" },
          ...
          <div className="w-full h-14 bg-gray-200 text-blue-900 flex items-center justify-between px-4 rounded-md shadow-sm hover:shadow-md transition-shadow duration-200 cursor-pointer"
            ...
            <div className="flex items-center gap-1 text-yellow-600">
              ...
              {isRentalLocationModalOpen ? (
                <MdOutlineArrowDropUp className="text-yellow-600 text-2xl" size={30} />
              ) : (
                <MdOutlineArrowDropDown className="text-yellow-600 text-2xl" size={30} />
              )}
            </div>
          </div>
...
>
  <div className="flex items-center gap-2">
    <MdLocationPin className="text-2xl font-bold text-red-500" />
    <span className="font-bold text-xl">States or Cities ({states.length})</span>
  </div>
  <div className="flex items-center gap-1 text-yellow-600">
    <span>View All</span>
    {isCityModalOpen ? (
