...
const page = () => {
  ...
  return (
    ...
    <Header headerTitle="1,100+ Car Rental Locations Worldwide" />
      ...
      <div className="max-w-7xl mx-auto   flex flex-col lg:flex-row lg:justify-between gap-4 items-center text-white">
        <h3 className="font-bold lgh:text-xl text-2xl">Car Rental Location</h3>
  ...
  return (
  <div className=" relative rounded-md bg-blue-600 hover:bg-violet-500 transition-colors duration-500 cursor-pointer " onClick={()=>setRegionModal(continent)} >
    <Image src={obj.image} alt="continent" className=" rounded-md border-[3px] border-violet-500" />
    <div className=" w-full h-fit my-3 mx-4">
    <h3 className=" font-bold text-xl">{continent}</h3>
    <h3 className=" font-semibold text-[20px]">{obj.countries.length} countries</h3>
    </div>
 {regionModal=== continent && <CountryDropDown contries={obj.countries} totalLocations={obj.totalLocations} rigion={continent}/>}
 </div>)
})}
      </div>
      </div>
...
function CountryDropDown({contries,totalLocations,rigion}:{contries:string[],totalLocations:number,rigion:string}) {
  return(
