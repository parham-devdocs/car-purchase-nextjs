...
const Page: FC<Props> = ({ params }) => {
  ...
  <ScheduleDropDown/>
        ...
        <div className=' flex items-start flex-wrap justify-between mt-6'>
          {services.map(service=>{return <LocationServicesDropdown dropDownClickHandler={(e)=>setDropDown(e)} dropDown={dropDown}  key={service.text} available={service.available} Icon={service.Icon} text={ service.text} title={service.title}/>})}
        </div>
      </div>
    </div>
  );
